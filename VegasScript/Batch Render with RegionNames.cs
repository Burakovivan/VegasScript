 ...
 *
 * Modified 02.11.2010 by AntonChik
...
public class EntryPoint
{
    ...
    public void FromVegas(Vegas vegas)
    {
        ...
        if (DialogResult.OK == result)
        {
            ...
            DoBatchRender(SelectedTemplates, outputFilePath, renderMode, UseChannelMapping.Checked);
        }
    }

    void DoBatchRender(ArrayList selectedTemplates, String basePath, RenderMode renderMode, bool useChannelMapping)
    {
        ...
        foreach (RenderItem renderItem in selectedTemplates)
        {
            ...
            if (RenderMode.Regions == renderMode)
            {
                ...
                foreach (ScriptPortal.Vegas.Region region in myVegas.Project.Regions)
                {
                    ...
                    // Render the region
                    status = DoRender(regionFilename, renderItem, region.Position, region.Length, useChannelMapping);
            ...
            else
            {
                ...
                status = DoRender(filename, renderItem, renderStart, renderLength, useChannelMapping);
            }
            if (RenderStatus.Canceled == status) break;
        }
    }
    ...
    RenderStatus DoRender(String filePath, RenderItem renderItem, Timecode start, Timecode length, bool useChannelMapping)
    {
        ...
        RenderStatus status = myVegas.Render(new RenderArgs(myVegas.Project)
        {
            OutputFile = filePath,
            RenderTemplate = renderItem.Template,
            Start = start,
            Length = length,
            UseChannelMapping = useChannelMapping
        });
    ...
    DialogResult ShowBatchRenderDialog()
    {
        ...
        UseChannelMapping = AddCheckboxControl(dlog,
                                            "Use channel mapping",
    ...
    CheckBox AddCheckboxControl(Form dlog, String labelName, int left, int top, bool enabled)
    {
        Label label = new Label();
        label.AutoSize = true;
        label.Text = labelName;
